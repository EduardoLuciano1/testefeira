...
app.get('/api/posts', (req, res) => {
    try {
        const posts = db.prepare(`SELECT id, author_email, content, created_at FROM posts ORDER BY created_at DESC LIMIT 100`).all();
...
app.post('/api/posts', requireAuth, (req, res) => {
    ...
    try {
        const stmt = db.prepare(`INSERT INTO posts (author_email, content) VALUES (?, ?)`);
        const info = stmt.run(req.user.email, content.trim());
        const post = db.prepare(`SELECT id, author_email, content, created_at FROM posts WHERE id = ?`).get(info.lastInsertRowid);
